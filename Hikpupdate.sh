...
# user_rules(){  
    wget --no-check-certificate -O /tmp/user-rules.txt https://raw.githubusercontent.com/kai258/KaiAD/master/kpupdate.sh
      if [ "$?"x != "0"x ]; then
        echo_date "下载自用规则失败"
        ...
        user_local=$(sed -n '1p' /etc/storage/koolproxy_rules_script.sh |  awk -F' ' '{print $3$4}'  | sed  's/-//g' | sed  's/://g')
        if [ "$user_online" -le "$user_local" ];then
           ...
           cp -f /tmp/user-rules.txt /etc/storage/koolproxy_rules_script.sh
           rm_cache;restart_kp
        fi
      fi  
# }